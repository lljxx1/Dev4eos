<template>
  ...
  >
    <header id="nav" v-if="!frameMode">
      <h1 id="logo-block">
        <router-link to="/">
          ...
          <span style="margin-left:5px" class="logo-name">Dev4eos</span>
        </router-link>
      </h1>
      <div id="actions">
        ...
        <ul class="right">
          ...
          <Dropdown class="dropdown action-item" v-if="false">
            <a class="dropdown-toggle ai-button" slot="button">
              <v-icon name="server" scale="0.8"/>CompileServer: eosmedinodes
            </a>
            <ul
              class="dropdown-menu dropdown-menu-actions round dropdown-menu-right"
              slot="dropdown"
            >
              <div class="dropdown-container net-settings complie-server">
                ...
                <div class="meta-info">
                  <a class="btn btn-link">eosmedinodes</a>
                </div>
                <p style="color:red">please vote these producer for provide compile service</p>

                <template v-if="false">
                  <h3>Provide Compile Server</h3>
                  <div class="form-info">
                    <p>please follow this project</p>
                  </div>
                </template>
              </div>
            </ul>
          ...
          <Dropdown class="dropdown action-item" v-if="projectType == 'fibos'">
            <a class="dropdown-toggle ai-button" slot="button">
              <v-icon name="server" scale="0.8"/>Import
            </a>
            <ul
              class="dropdown-menu dropdown-menu-actions round dropdown-menu-right"
              slot="dropdown"
            >
              <div class="dropdown-container net-settings complie-server">
                ...
                <div class="meta-info">
                  <p>
                    <textarea v-model="stateDatabase" style="height: 573px;"></textarea>
                  </p>
                  <a class="btn btn-primary" @click="exportState()">Export</a>
                  <a class="btn btn-primary" @click="importState()">Import</a>
                </div>
              </div>
            </ul>
          </Dropdown>
          ...
          <Dropdown class="dropdown action-item" v-if="projectType == 'fibos'">
            <a class="dropdown-toggle ai-button" slot="button">
              <v-icon name="server" scale="0.8"/>Local Account
            </a>
            <ul
              class="dropdown-menu dropdown-menu-actions round dropdown-menu-right"
              slot="dropdown"
            >
              <div class="dropdown-container net-settings complie-server">
                ...
                <div class="meta-info">
                  <p>
                    <input type="text" v-model="localConfig.account" placeholder="Account">
                  </p>
                  <p>
                    <input type="text" v-model="localConfig.privateKey" placeholder="PrivateKey">
                  </p>
                  <a class="btn btn-primary" @click="saveLocalConfig()">Save</a>
                </div>
              </div>
            </ul>
          </Dropdown>
          ...
          <Dropdown class="dropdown action-item dropdown-network-selector">
            <a class="dropdown-toggle ai-button" slot="button">
              ...
              <template v-if="projectType == 'fibos'">TestNet</template>
            ...
            >
              <div class="dropdown-container net-settings">
                ...
                <div class="network-list-body" v-if="networkShowMode">
                  ...
                  <div class="meta-info" v-if="projectType == 'fibos'">
                    <p>Chain ID: 68cee14f598d88d340b50940b6ddfba28c444b46cd5f33201ace82c78896793a</p>
                    <p>
                      Endpoint:
                      <select v-model="currentEndpoint">
                        <option
                          v-for="endpoint in allEndpoints"
                          :value="currentEndpoint"
                        >{{ currentEndpoint.ssl_endpoint }}</option>
                      </select>
                      <!-- {{ currentEndpoint.ssl_endpoint }} -->
                    </p>
                    <p>Ironman -> Settings -> NetWorks -> new (import this)</p>
                  </div>
                  ...
                  <div class="form-info" v-if="projectType == 'fibos'">
                    <h3>Create Account</h3>
                    <!-- <p><input type="text" placeholder="Account (12 char)" v-model="registerAccount"></p> -->
                    <p>
                    ...
                    <!-- <p v-if="privateKey">Private Key: {{ privateKey }}  <span class="notify">(save this)</span> -->
                    <a class="btn btn-primary" @click="generateKeyPair()">Create Keypair</a>
                    <a
                      class="btn btn-primary"
                      href="http://103.80.170.236/#/createaccount"
                      target="_blank"
                    >Create Account</a>
                  </div>
          ...
          <Dropdown class="dropdown dropdown-user action-item">
            <a class="dropdown-toggle ai-button" slot="button">
              <template v-if="projectType == 'fibos'">
                <img src="./../assets/ironman.png">
                <span class="el-dropdown-link" @click="connectScatter()">
                  <span v-if="scatter && !identity">Attach an Account</span>
                  <span v-if="scatter && identity">{{ identity.name }}</span>
                  <span v-if="!scatter">
                    <a href="http://foironman.com/" target="_blank">Install Ironman</a>
                  </span>
                </span>
              </template>
        ...
        <div class="pannel-main">
          <div class="pannel-piece pannel" style="width: calc(33.3333% - 0.5px);">
            <div class="windowLabelCont">
              <span class="windowLabel" v-if="projectType == 'fibos'">Javascript</span>
    ...
    <div id="bottom-bar" v-if="!frameMode">
      <span v-if="projectType == 'eos'">
        <a href="https://dev4eos.com/qrcode.jpg" target="_blank">wechat group</a>
      </span>

      <ul class="right" v-if="projectType == 'eos'">
        </li>-->
        <li>
          <a
            class="dropdown-toggle ai-button"
            title="issues or request new feature"
            href="https://github.com/lljxx1/Dev4eos/issues"
            target="_blank"
          >
        ...
        <Dropdown class="dropdown action-item dropup" v-if="false">
          <a class="dropdown-toggle ai-button" slot="button">
            <v-icon name="question-circle" scale="0.8"/>
          </a>
          <ul class="dropdown-menu dropdown-menu-actions round dropdown-menu-right" slot="dropdown">
            <div class="dropdown-container net-settings">
              <h3 style="margin: 0 0 15px;">NetWorkInfo</h3>
              <div class="meta-info">
                <p>Chain ID: 68cee14f598d88d340b50940b6ddfba28c444b46cd5f33201ace82c78896793a</p>
                <p>Endpoint: http://103.80.170.236:8870</p>
                <p>Ironman -> Settings -> NetWorks -> new (import this)</p>
              </div>
            </div>
          </ul>
        </Dropdown>
        <!-- <li><v-icon name="question-circle" scale="0.8"/></li> -->
      </ul>
    </div>
...
var projectType =
  window.location.href.indexOf("fodev.info") > -1 ? "fibos" : "eos";
...
if (projectType == "fibos") {
  currentNetwork = "fotestnet";
}
...
const foTestNetwork = {
  ...
  protocol: "https",
  port: 443,
  host: "rpc-testnet.ilovefibos.com",
  blockchain: "fibos",
...
var testnet = axios.create({
  baseURL: "http://103.80.170.236",
  timeout: 1000,
  headers: {}
});

var complier = axios.create({
  baseURL: "https://compiler.dev4eos.com:8081",
  timeout: 300 * 1000,
  headers: {}
});
...
export default {
  ...
  data() {
    return {
      ...
      complieServerList: [
        {
          host: "https://compiler.dev4eos.com:8081"
        }
      ],
  ...
  mounted: function() {
    ...
    if (document) {
      ...
      if (this.projectType == "eos") {
        ...
        window.scatter.connect("Dev4eos").then(connected => {
        ...
        console.log("fibos window.ironman");
        self.scatter = window.ironman;
        if (window.scatter) self.identity = window.ironman.identity;
        document.addEventListener("ironmanLoaded", function() {
          console.log("ironmanLoaded", window.ironman.identity);
          self.scatter = window.ironman;
          self.identity = window.ironman.identity;
        });
      }
      ...
      try {
        if (self.identity) {
          self.identity.accounts.forEach(function(x) {
            if (x.blockchain == "eos" || x.blockchain == "fibos") {
          ...
          if (this.projectType == "eos") {
            ...
            self.eosClient = window.ironman.fibos(
              foTestNetwork,
              Eos,
              {
                broadcast: true,
                chainId: foTestNetwork.chainId
              },
              "https"
            );
          }
...
EOSIO_ABI( hello, (hi) )`;
  ...
  console.log(res);
    ...
    popupVote() {
      ...
      this.eosClient
        ...
        .catch(function(error) {
          ...
          self.dialogMessage =
            "proxy info submit failed. <br> <span style='color:red'>" +
            error.error.details[0].message.split(":")[1] +
            "</span>";
    ...
    voteProducer() {
      ...
      this.newProducers = ["fibosisfunoo"];
    ...
    connectScatter: function() {
      ...
      if (this.projectType == "fibos") {
        console.log("connectScatter ironman");
        this.scatter
          .getIdentity({
            accounts: [
              {
